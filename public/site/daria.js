...
(() => {
  const canvas = document.querySelector("[data-droplets-canvas]");
  ...
  const setPointerFromEvent = (event) => {
    ...
    pointer.x = ((x - bounds.left) / bounds.width) * 2 - 1;
    pointer.y = -(((y - bounds.top) / bounds.height) * 2 - 1);
  };

  const render = (time = 0) => {
    const elapsed = (time - startTime) * 0.001;

    if (!hasPointerInput && window.innerWidth <= 700 && !reducedMotion.matches) {
      pointer.x = 0.68 + Math.sin(elapsed * 0.31) * 0.16;
      pointer.y = -0.08 + Math.cos(elapsed * 0.27) * 0.18;
    }
  ...
  window.addEventListener("pagehide", () => {
    ...
    window.removeEventListener("touchmove", setPointerFromEvent);
    window.removeEventListener("touchstart", setPointerFromEvent);
